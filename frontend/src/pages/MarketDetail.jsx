...
export default function MarketDetail({ user }) {
  ...
  const fetchItem = async () => {
    try {
      setLoading(true);
      const data = await getProduct(id); // MarketPost 1건
      setItem(data || null);
    } catch (e) {
  ...
  // 대표 이미지: isPrimary=true가 우선, 없으면 첫 번째 이미지
  const mainImage =
    (item.images &&
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto bg-white rounded shadow">
      <div className="flex items-start space-x-6">
        ...
        <div className="flex-1">
          ...
          <div className="text-sm text-gray-600 mb-4">
            판매자 ID:{" "}
            <span className="font-medium">{item.authorId ?? "-"}</span>
            <span className="ml-4">
              카테고리:{" "}
      ...
      <div className="mt-8">
        ...
        <CommentList itemId={item.id} user={user} />
      </div>
    </div>
  );
}
