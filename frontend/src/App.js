...
function AppContent() {
 ...
 useEffect(() => {
  ...
  (async () => {
    try {
      console.log("App useEffect 실행 - getCurrentUser 호출");
      const u = await getCurrentUser();
      console.log("getCurrentUser 결과:", u);
      if (!mounted) return;
      if (u) {
...
}, []); // 빈 배열 유지 - 컴포넌트 마운트 시에만 실행

// location 변경 시 사용자 상태 확인
useEffect(() => {
  const checkUser = async () => {
    console.log("checkUser 실행 - pathname:", location.pathname);
    const u = await getCurrentUser();
    console.log("checkUser - u:", u);
    console.log("checkUser - 현재 user:", user);
    
    if (u) {
      setUser(u);
      console.log("checkUser - setUser 호출");
    } else {
      setUser(null);
    }
  };
