// ✅ 로고 이미지
import title from "../images/title.png";
import logo from "../images/logo.png";
import KakaoMap from "./KakaoMaps"; // ✅ 카카오맵 컴포넌트 불러오기

export default function MapPage() {
  return (
    <div className="p-8 text-center bg-gray-50 min-h-screen">
      <h1 className="text-4xl font-extrabold text-blue-600 mb-4">
        <img
          src={logo}
          alt="Title"
          className="inline-block h-16 w-auto mr-3 align-middle"
        />
        <span className="align-middle">인하공전 캠퍼스 지도</span>
        <img
          src={logo}
          alt="Title"
          className="inline-block h-16 w-auto ml-3 align-middle"
        />
      </h1>

      <p className="text-gray-600 mb-6">
        이 페이지는 로그인 없이 누구나 접근할 수 있는 기본 홈 화면입니다.
      </p>

      <div className="bg-white shadow-md p-6 rounded-xl max-w-3xl mx-auto border border-gray-200">
        <p className="text-gray-500 text-lg">
          아래는 <strong>카카오 지도 API</strong>를 연동한 실제 지도입니다.
        </p>

        {/* ✅ 여기에 카카오맵 컴포넌트 삽입 */}
        <KakaoMap />

        <p className="mt-4 text-sm text-gray-400">
          지도 위에 캠퍼스 건물 마커, 경로 등이 추가될 예정입니다.
        </p>
      </div>
    </div>
  );
}
